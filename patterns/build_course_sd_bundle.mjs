...
const bundleDir = path.join(patternsDir, 'course_sd');
const manifestPath = path.join(patternsDir, 'course_sd_manifest.md');
...
Copy the **40 \`.pat\` files inside \`patterns/course_sd/\` directly to the
root of the controller SD card**. Do not copy the enclosing directory, and do
not mix these files with per-protocol pattern folders.
...
promotion, but they are not standalone SD-card bundles: use \`course_sd\` for
the controller card.
