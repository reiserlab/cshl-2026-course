...
released for instructor validation. Cue A is always reinforced, the default
conditional LED level is 25%, and the default activation stock is **hot cell**.
P1, moowalker, and other validated activation lines are reasonable alternatives
...
The conditional LED feature is web-runner-only and remains subject to the
physical-rig bench gate below.
...
Edit the two linked variables near the top of whichever P3 YAML file you plan
to run. Editing both values is essential: `pattern_name` is the human-readable
file lookup, while
`pattern_ID` is the numeric fallback used by the shared SD-card bundle. A
mismatch can silently display a different stimulus if the name lookup fails.

| Choice | `pattern_name` to enter | `pattern_ID` to enter | Shared SD file | Student-facing stimulus |
| ---: | --- | ---: | --- | --- |
| 1 | `p3_heisenberg_ts` | 36 | `036_p3_heisenberg_ts.pat` | Classic upright/inverted T figures |
| 2 | `p3_heisenberg_high_low` | 37 | `037_p3_heisenberg_high_low.pat` | High versus low horizontal bars |
| 3 | `p3_heisenberg_slashes` | 38 | `038_p3_heisenberg_slashes.pat` | +45° versus −45° slashes |
| 4 | `p3_heisenberg_relational` | 39 | `039_p3_heisenberg_relational.pat` | Relational upper/lower slash pair |
| 5 | `p3_dill_random_checkers` | 40 | `040_p3_dill_random_checkers.pat` | Dill-style randomized square textures |
...
The released YAMLs expose two linked variables:

```yaml
pattern_name: &pattern_name "p3_heisenberg_ts"
pattern_id: &pattern_id 36
```

Change both values using the table above, in whichever short or full file you
plan to run. The YAML variable is written `pattern_id`, and it supplies the
command field `pattern_ID`. Arena Studio resolves the pattern by name and uses
`pattern_ID` as the SD-card fallback; keeping the pair matched prevents a
...
quadrants. The released default is **25%** with no hysteresis:
...
---
*Updated 2026-07-10 01:47 ET. Pattern source: `patterns/p3_conditioning/`; shared
