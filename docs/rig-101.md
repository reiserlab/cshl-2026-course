...
## Hardware files and bill of materials

The hardware files and bill of materials for the G6 fly-on-ball walking setup
are in the Fly-Lab-Gear repository:
...
The BOM is useful for identifying parts, but students do not need to source
anything during the course. It lists the rig as a G6 fly-on-ball walking setup
mounted on a 12" × 12" breadboard, with the arena, optostim path, camera,
treadmill, illumination, heater, and fly-holder parts broken out as
subassemblies.

## The LED arena (the panels)

 (fix this: section should say there are 9 columns of panels since we left one open directly behind the fly for the camera cables to come out.)
